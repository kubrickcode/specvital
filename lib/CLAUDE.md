...
3. Require blank import: `_ "github.com/kubrickcode/specvital/lib/parser/strategies/{name}"`
...
- **Public API** (`parser/`): Breaking changes require major version bump
- **Internal** (`parser/{strategies,detection,framework,tspool}/`): Safe to refactor
...
1. Create `parser/strategies/{framework}/definition.go`
...
```
lib/
├── crypto/           # Cryptography utilities (encryption)
├── parser/
