...
- **Domain Layer**는 외부 의존성이 없음
- **UseCase Layer**는 Domain 인터페이스에만 의존
- **Adapter Layer**는 Infrastructure를 사용하여 Domain 인터페이스 구현
- **Handler Layer**는 외부 요청을 UseCase 호출로 변환
- **Application Layer**는 의존성을 연결
...
- Adapter 레이어는 정책이 아닌 **기술적 실행**에 집중
...
- [ADR-01: 코어 라이브러리 분리](/ko/adr/core/01-core-library-separation.md) (Core)
