...
> 🇺🇸 [English Version](/en/adr/collector/05-oauth-token-graceful-degradation.md)
...
저장소 분석 서비스는 공개 및 비공개 저장소 모두 지원해야 한다:
...
이들은 다른 처리 전략이 필요하다—모든 토큰 에러를 동일하게 처리하면 사용자를 불필요하게 차단하거나 실제 문제를 가린다.
