...
# ADR-09: 스케줄 기반 재수집 아키텍처

> :us: [English Version](/en/adr/09-scheduled-recollection.md)
...
ADR-05에서 초기 분석 요청을 위한 큐 기반 비동기 처리를 확립했다. 이는 장시간 실행 작업 문제를 해결하지만, 지연이 발생한다: 이전에 분석한 저장소를 요청할 때도 큐 처리를 기다려야 한다.

**저장소가 미리 분석되어 최신 상태로 유지되면, 사용자는 분석 대기 없이 즉각적인 응답을 받는다.**
...
핵심 통찰: **대부분의 사용자 요청은 이전에 분석한 저장소에 대한 것이다**. 사전 수집은 대다수 요청에서 큐 대기 시간을 제거한다.

### 부가적 이점: 데이터 신선도

응답 시간 외에도, 사전 수집은 데이터 노후화도 해결한다 (의존성 업데이트, 보안 패치, 코드 리팩토링).
...
2. **분산 락**: 단일 실행 보장을 위한 Redis 기반 락
...
저장소 가시성은 데이터베이스에 저장하지 않는다. 언제든 변경될 수 있기 때문이다 (public↔private). 대신 clone 시점에 런타임으로 검증한다.
...
**참고:** 저장된 사용자 토큰으로 private 저장소 재수집이 기술적으로는 가능하다. 의도적으로 제외한 이유:
