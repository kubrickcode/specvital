...
func findAttributeArgumentList(attr *sitter.Node) *sitter.Node {
	for i := 0; i < int(attr.ChildCount()); i++ {
		child := attr.Child(i)
		if child.Type() == dotnetast.NodeAttributeArgumentList {
			return child
		}
	}
	return nil
...
func getDisplayNameFromAttribute(attr *sitter.Node, source []byte) string {
	argList := findAttributeArgumentList(attr)
	if argList == nil {
		return ""
	}
	...
	for i := 0; i < int(argList.ChildCount()); i++ {
		...
		if arg.Type() == dotnetast.NodeAttributeArgument {
			name, value := parseAssignmentExpression(arg, source)
			if name == "DisplayName" {
				return value
			}
		}
...
func isSkipped(attr *sitter.Node, source []byte) bool {
	...
	for i := 0; i < int(argList.ChildCount()); i++ {
		arg := argList.Child(i)
		if arg.Type() == dotnetast.NodeAttributeArgument {
			name, _ := parseAssignmentExpression(arg, source)
			if name == "Skip" {
				return true
			}
		}
...
func parseAssignmentExpression(argNode *sitter.Node, source []byte) (string, string) {
	for i := 0; i < int(argNode.ChildCount()); i++ {
		...
		if child.Type() == dotnetast.NodeAssignmentExpression {
			...
			for j := 0; j < int(child.ChildCount()); j++ {
				...
				case dotnetast.NodeStringLiteral:
					value = dotnetast.ExtractStringContent(part, source)
				}
			}
			return name, value
		}
