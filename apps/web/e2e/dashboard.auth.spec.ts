...
test.describe("Dashboard Page (Authenticated)", () => {
  ...
  test("should sort repositories by different criteria", async ({ page }) => {
    ...
    // Find and click sort dropdown - uses button with name "Sort"
    ...
    // Verify "Recent" is checked by default
    await expect(page.getByRole("menuitemradio", { name: "Recent" })).toHaveAttribute(
      "aria-checked",
      "true"
    );
    ...
    // Open dropdown again to verify selection changed
    await sortButton.click();
  ...
  test("should filter repositories by ownership", async ({ page }) => {
    // Wait for content to fully load
    await expect(
      page
        .getByRole("heading", { name: /no repositories yet/i })
    ...
    // Find ownership filter dropdown
    const ownershipButton = page.getByRole("button", { name: /filter by ownership/i });
    const hasOwnershipButton = await ownershipButton.isVisible({ timeout: 5000 }).catch(() => false);

    if (!hasOwnershipButton) {
      test.skip();
      return;
    }
    ...
    await expect(page.getByRole("menuitemradio", { name: "Mine" })).toBeVisible();
    await expect(page.getByRole("menuitemradio", { name: "Organization" })).toBeVisible();
    await expect(page.getByRole("menuitemradio", { name: "Others" })).toBeVisible();

    // Verify "All" is checked by default
    await expect(page.getByRole("menuitemradio", { name: "All" })).toHaveAttribute(
      "aria-checked",
      "true"
    );
    ...
    // Result should show filtered repos or empty state message
    // Just verify the filter was applied (page doesn't error)
  ...
  test("should search repositories", async ({ page }) => {
    ...
    // Search should filter - either shows results or empty state
    await expect(
      page
        .getByRole("button", { name: /add bookmark|remove bookmark/i })
        .first()
        .or(page.getByText(/no search results/i))
    ...
    // Verify repositories are shown again
    await expect(
